...
function App() {
  ...
  useEffect(() => {
    if (+bill > 0 && +tip > 0 && +numberOfPeople > 0) {
      setTotalTip(+bill * (+tip / 100) / +numberOfPeople);
      setTotalMoney(((+bill * (+tip / 100)) + +bill) / +numberOfPeople);
    }
  }, [bill, tip, numberOfPeople]);

  const onBillChange = (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const onPeopleChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    <div className="container">
      ...
      <div className="tip__calculator">
        ...
        <div className='calculator__result'>
          <Result title="Tip Amount" amount={totalTip} />
          <Result title="Total" amount={totalMoney} />
